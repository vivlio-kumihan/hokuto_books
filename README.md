...
* 問い合わせフォーム　確認、送信完了のメッセージのやり方 => プラグインを変更して解決させるとのこと。
*　当院からのお知らせが現状のJSではまずい。
  カテゴリーを選択したら上から三つ分の高さを計算して出現させるように変更する。
* PC版でホバーのアニメーションをSmp版ではキル設定。mixinは作ったがやり方分かってない。_hover-pc.sass
* 最初のページでローディングが終了してからのアニメーション
* policyのアニメーション
* 外来受診のアニメーション
