...
原稿は、
`5800/営業部/相生/☆北斗書房/ブログ`

データは、`5800/営業部/相生/☆北斗書房/☆新・北斗書房チラシ/`

出力物としては、
* A3 駅のポスター => 社内のプリンターで出力
* A1 ポスター => pdfを相生主任の入稿フォルダに入れる。（プロッターで出力される。）


* pdfX4で圧縮は全てなしにして出力。

相生主任に確認してもらう。
確認後　6部出力　枠のあたりがついたポスターのpdfを相生主任の入稿フォルダに入れる。

やることは、
* top.phpに貼ってあるバナーに投稿へのリンクを貼る。
* 投稿のトップに表示されるポスター画像のリンクを変更する。`css`の作業としては、`align center => aling left`へ変更

## ファイルのある場所
`5800/WEB/北斗書房サイト情報・・・/北斗書房サイト一式/北斗書房データ/サイトアップ用バナー
`
> ffftpのアプリを使うよう指示。=> macでは使えないので代替としてCyberDuckを使う。

### 北斗書房を置いているプロバイダーへ接続
* `北斗書房/コントロールパネル`からログイン
* `公開サイト用設定/FTPファイル管理/FTP接続制限`の設定

* `IPアドレス`をコピーして、
設定の編集ボタンを押し設定画面へ
`192.168.100.49`
* ダイアログの下に最新のアドレスがあるからそれをコピー
ダイアログ内のIPアドレスの下段にペーストする。

絶対に忘れてはいけないこと。=>
**作業終了後は、追加したアドレスを削除する。**


## CPIコンパネ
ログインして現在接続しているアドレスを追加してFTPを使えるようにする。

#### アクセスする
* URL: https://aa112.secure.ne.jp/admin/
* ID: aa11215vkt
* PW: uFpZLYnS

defaultは、`58.70.14.38`

my mac: 192.168.100.55
```
公開サイト用設定 > FTP/ファイル管理 > FTP接続制限の設定 > 「設定を編集」ボタン
```

ダイアログ最下段に表示される現在の自身のアドレスをコピー
```
あなたの現在の接続IPアドレスは、XXX.XXX.XXX.XXXです。
```

上の`textarea`の２行目へペースト

これでFTPが使えるようになる。

## 編集

* `5800/営業部/相生/☆北斗書房/☆新北斗書房チラシ/`の中の、<br />
`2024XX_北斗書房ポスター店頭用＆お知らせ用画像`のファイルを編集する。
  * イラストレータのデータをリネームして編集。キャッチ・バナー用の画像を書き出す。
...
#### 編集
* 最新の投降をコントロールを押しながら`新しいタブで開く`。
* タイトルの月を変更。
* 本文最上段を一行追加して空行の状態でカーソルを置く。
* `メディアを追加`ボタンを押し、`ファイルをアップロード`して`メディアライブラリ`から新規キャッチを選択。
* 本文最上段に追加されたp要素 > img要素のclass属性の`aligncenter`パラメーターを`alignnone`に変更。
* 本文の日付を現行通りに変更。
* パーマリンクをコピーしておく。
  * （例）http://www.hokutoshobo.jp/news/info/2535/
  * 9月
    http://www.hokutoshobo.jp/news/?p=2592&preview=true
    http://www.hokutoshobo.jp/news/?p=2620&preview=true
    http://www.hokutoshobo.jp/news/files/2024/09/baner_soudannkai202410-topImage.jpg
    http://www.hokutoshobo.jp/news/info/2631/
* 公開。


## CSS・PHPファイルの編集

* `NR5800M/WEB/北斗書房サイト情報＆パーツ：夜西さんより/` <br /> `北斗書房サイト一式/北斗書房パーツ/+サイトアップ用バナー/`の中の、<br />
`home-top.php`、`page.css`を編集する。

#### home-top.php L10

```
<a href="http://www.hokutoshobo.jp/news/info/2535/"><li class="top_menu1"><!-- 相談会バナー -->
```
このhref属性のパラメーターをコピーしたURLに変更する。

#### page.css L160

```
.top_menu ul li.top_menu1 {background:url(../img/baner_soudannkai202312.jpg) no-repeat right bottom;}	/*相談会バナー*/
```
このbackground属性のファイル名を書き出したものに変更する。
...
#### ファイルの送信
* common/cssにアクセスして`page.css`を差し替える。
* common/imgにアクセスしてバナー画像を入れる。
* wp-content/themes/hokutoshoboにアクセスして`home-top.php`を差し替える。
