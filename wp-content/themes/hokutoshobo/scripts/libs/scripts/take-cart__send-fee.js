...
class SendFee {
  ...
  feeCalcMth(prefName) {
    ...
    let sendFee = 0;
    if (prefName !== null) {
      let direction = Object.keys(this.AREA).find(key => this.AREA[key].includes(prefName));
      const totalWeight = orderedItems.reduce((acc, obj) => acc + obj["重量小計"], 0);
      if (totalWeight < 51) {
